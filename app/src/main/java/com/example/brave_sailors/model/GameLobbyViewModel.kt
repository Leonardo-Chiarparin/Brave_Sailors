    ...
    private fun observeFriendsFromDatabase() {
        viewModelScope.launch {
            friendDao.getAllFriendsFlow().collect { friendEntities ->
                ...
                syncFriendsDetails(friendEntities)
                _uiState.update { it.copy(friends = dbFriends) }
            }
        }
    }

    private fun syncFriendsDetails(friends: List<FriendEntity>) {
        viewModelScope.launch(Dispatchers.IO) {
            friends.forEach { friend ->
                usersRef.child(friend.id).child("profile").child("info")
                    .addListenerForSingleValueEvent(object : ValueEventListener {
                        override fun onDataChange(snapshot: DataSnapshot) {
                            val remoteName = snapshot.child("name").getValue(String::class.java)
                            if (!remoteName.isNullOrEmpty() && remoteName != friend.name) {
                                launch(Dispatchers.IO) {
                                    friendDao.insertFriend(friend.copy(name = remoteName))
                                }
                            }
                        }
        ...
        viewModelScope.launch {
            ...
            matchmakingListener = object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    if (timestamp != null && Math.abs(now - timestamp) < READY_TIMEOUT_MS) {
