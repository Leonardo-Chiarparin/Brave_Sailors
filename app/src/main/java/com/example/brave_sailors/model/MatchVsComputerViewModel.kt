...
class MatchVsComputerViewModel(
    ...
    fun onPlayerFire(row: Int, col: Int) {
        ...
        val aiAlive = countShipsAlive(state.aiGrid)
        val aiComposition = calculateFleetComposition(state.aiGrid)
        val isWin = (aiAlive == 0)
        ...
        while (!state.isPlayerTurn && !state.isGameOver && state.shotsRemaining > 0) {
            ...
            if (difficulty == "Hard") {
                try {
                    ...
                    val aiMove = AiNetworkClient.service.getNextMove(GridRequest(simpleGrid, difficulty))
                    coords = aiMove.row to aiMove.col
                } catch (e: Exception) {
                    coords = calculateAiCoordinates(state.playerGrid)
                }
            } else {
                coords = calculateAiCoordinates(state.playerGrid)
            }
    ...
    private fun performShot(r: Int, c: Int) {
        ...
        val pComposition = calculateFleetComposition(state.playerGrid)
    ...
    private fun calculateAiCoordinates(grid: List<List<GridCell>>): Pair<Int, Int> {
        ...
        return when (difficulty) {
            "Easy" -> getRandomCoords()
            "Normal" -> getRandomCoords()
            "Hard" -> getHardCoords()
            else -> getRandomCoords()
        }
    }
    ...
    private fun getHardCoords(): Pair<Int, Int> {
